// src/components/JobSeekersWorkflow/MobileWorkflow.jsx
...
const MobileWorkflow = () => {
  ...
  const handleSwipe = () => {
    if (index < jobSeekerSteps.length - 1) {
      setIndex(index + 1);
    }
  };
  ...
  return (
    <section className="lg:hidden w-full bg-white dark:bg-black py-12">
      ...
      <div className="relative w-full flex justify-center items-center h-[360px]">
        <AnimatePresence>
          {jobSeekerSteps.slice(index, index + 3).map((step, i) => {
            const isTop = i === 0;
            const Icon = step.icon;
            const isLastCard = index === jobSeekerSteps.length - 1 && isTop;

            return (
              <motion.div
                key={step.title}
                className={`absolute w-[300px] h-[340px] rounded-[25px] shadow-2xl p-6 text-white flex flex-col items-center justify-center gap-4 ${step.color} ${
                  isLastCard ? "cursor-pointer" : ""
                }`}
                style={{ zIndex: jobSeekerSteps.length - i }}
                initial={{
                  scale: 0.9 - i * 0.05,
                  y: i * 15,
                  rotate: -i * 4,
